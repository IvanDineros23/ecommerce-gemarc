...
<div class="container py-4">
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-lg-8">
            ...
            @php
                // Error bag for password update (Jetstream/Fortify)
                $passwordErrorBag = $errors->updatePassword ?? null;
                $hasPasswordError = $passwordErrorBag && $passwordErrorBag->any();

                // Primary source: session('status'). Fallback: explicit query param
                $status = session('status') ?: (request()->query('password_updated') ? 'password-updated' : null);
                $hasSuccessStatus = $status ? true : false;
                $hasGenericSuccess = session('success') ? true : false;
            @endphp

            <!-- GLOBAL TOAST -->
            <div 
                x-data="{ show: {{ ($hasSuccessStatus || $hasGenericSuccess || $hasPasswordError) ? 'true' : 'false' }} }"
                x-show="show"
                x-transition
                ...
                @if ($hasSuccessStatus)
                    <div class="alert alert-success shadow">
                        ...
                        @if ($status === 'password-updated')
                            Your password has been updated and an email notification was sent.
                        @else
                            {{ $status }}
                        @endif
                    </div>
                @elseif ($hasGenericSuccess)
                    <div class="alert alert-success shadow">
                        <strong>Success!</strong> {{ session('success') }}
                    </div>
                @elseif ($hasPasswordError)
                    <div class="alert alert-danger shadow">
                        <strong>Error!</strong> {{ $passwordErrorBag->first() }}
                    </div>
                @endif
            </div>
                ...
                <div class="card-body p-4" x-show="open">
                    <form x-data="{ email: '', confirm: '', match: true }" @input="match = (email === confirm)" method="POST" action="{{ route('settings.saveBasicInfo') }}">
                        ...
                        @if(!$isEmployee)
                            ...
                            <button type="submit" class="btn btn-success" :disabled="!match || !email || !confirm" :class="{'opacity-50': !match || !email || !confirm}">
                                Save Basic Info
                            </button>
                        @endif
                    </form>
                    ...
                    >
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input :type="showCurrent ? 'text' : 'password'" id="current_password" name="current_password" class="form-control" placeholder="Current Password" required autocomplete="current-password">
                                <button type="button" @click="showCurrent = !showCurrent" tabindex="-1" class="btn btn-outline-secondary" style="border-left:0;">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input :type="showNew ? 'text' : 'password'" id="new_password" name="password" x-model="password" class="form-control" placeholder="New Password" required autocomplete="new-password">
                                <button type="button" @click="showNew = !showNew" tabindex="-1" class="btn btn-outline-secondary" style="border-left:0;">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input :type="showConfirm ? 'text' : 'password'" id="confirm_password" name="password_confirmation" class="form-control" placeholder="Confirm New Password" required autocomplete="new-password">
                                <button type="button" @click="showConfirm = !showConfirm" tabindex="-1" class="btn btn-outline-secondary" style="border-left:0;">
